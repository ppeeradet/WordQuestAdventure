...
function stopStoryAudio(){if(storyAudio){storyAudio.pause();storyAudio.currentTime=0}if('speechSynthesis'in window)speechSynthesis.cancel()}
function showStoryDialog(story,buttonLabel){stopStoryAudio();storyAudioId=story.audioId;$('#story-audio-status').textContent='';$('#story-title').textContent=story.boss?`👑 ${story.title}`:story.title;$('#story-en').textContent=story.en;$('#story-th').textContent=story.th;$('#story-continue').textContent=buttonLabel;const dialog=$('#story-dialog');if(!dialog.open)dialog.showModal()}
$('#story-continue').addEventListener('click',()=>{stopStoryAudio();$('#story-dialog').close()});
$('#story-dialog').addEventListener('close',stopStoryAudio);
$$('[data-story-speak]').forEach(button=>button.addEventListener('click',async()=>{const lang=button.dataset.storySpeak,text=lang==='en'?$('#story-en').textContent:$('#story-th').textContent,status=$('#story-audio-status');stopStoryAudio();if(lang==='th'){
  storyAudio=new Audio(`assets/story-th/${storyAudioId}.mp3`);
  storyAudio.preload='auto';storyAudio.onended=()=>{status.textContent=''};storyAudio.onerror=()=>{status.textContent='เล่นเสียงไม่ได้ กรุณาตรวจการเชื่อมต่อแล้วลองอีกครั้ง'};
  status.textContent='กำลังเปิดเสียงภาษาไทย…';
  try{await storyAudio.play();status.textContent='🔊 กำลังอ่านภาษาไทย'}catch(e){status.textContent='เล่นเสียงไม่ได้ กรุณาตรวจระดับเสียงแล้วลองอีกครั้ง'}
  return;
}if(!('speechSynthesis'in window)){status.textContent='อุปกรณ์นี้ไม่รองรับเสียงอ่านภาษาอังกฤษ';return}const utterance=new SpeechSynthesisUtterance(text);utterance.lang='en-US';utterance.rate=.84;utterance.onend=()=>{status.textContent=''};utterance.onerror=()=>{status.textContent='เล่นเสียงภาษาอังกฤษไม่ได้ กรุณาลองอีกครั้ง'};status.textContent='🔊 กำลังอ่านภาษาอังกฤษ';speechSynthesis.speak(utterance)}));
...
function renderQuestion(){
  const q=state.session[state.index];state.selected=null;state.typed='';state.spellingGuesses=[];state.spellingLastGuess=null;state.analysisWrong=[];state.analysisMisses=0;state.analysisLastGuess=null;state.hintCost=0;state.checked=false;
...
function renderKeyboard(){
  const q=state.session[state.index],letters='QWERTYUIOPASDFGHJKLZXCVBNM',last=state.analysisLastGuess,misses=state.analysisMisses||0,exhausted=misses>=5;
  ...
  const status=exhausted?'❌ ผิดครบ 5 ครั้งแล้ว · กดดูคำตอบเพื่อเรียนรู้คำนี้':last?last.correct?`✅ ถูกต้อง! เติม ${last.letter} แล้ว`:`❌ ${last.letter} ยังไม่ใช่ตัวถัดไป`:'กดตัวอักษรเพื่อเติมคำทีละตัว';
  const wrongList=state.analysisWrong.length?`<div class="wrong-letters">ตัวที่ผิดในช่องนี้: <strong>${state.analysisWrong.join(' · ')}</strong></div>`:'';
  $('#answers').innerHTML=`<div class="typed-word" id="typed-word" aria-label="สะกดได้ ${state.typed.length} จาก ${q.answer.length} ตัว">${slots}</div><div class="letter-status ${last&&!last.correct?'letter-wrong':''}" role="status">${status}<span class="miss-count">ผิด ${misses}/5 ครั้ง</span>${wrongList}</div><div class="keys">${[...letters].map(l=>`<button type="button" data-letter="${l}" class="${state.analysisWrong.includes(l)?'letter-wrong':last?.correct&&last.letter===l?'letter-correct':''}" ${state.analysisWrong.includes(l)||state.checked||exhausted?'disabled':''}>${l}</button>`).join('')}<button type="button" class="key-delete" data-delete ${state.typed.length===0||state.checked||exhausted?'disabled':''}>⌫</button></div>`;
  $$('[data-letter]').forEach(button=>button.addEventListener('click',()=>guessAnalysisLetter(button.dataset.letter)));
  $('[data-delete]').addEventListener('click',()=>{if(state.checked||state.analysisMisses>=5)return;state.typed=state.typed.slice(0,-1);state.analysisWrong=[];state.analysisLastGuess=null;$('#continue-btn').disabled=true;setLetterFeedback('ลบตัวสุดท้ายแล้ว ลองสะกดต่อ','');renderKeyboard()});
}
function guessAnalysisLetter(letter){
  const q=state.session[state.index];if(!q||q.mode!=='analysis'||state.checked||state.analysisMisses>=5||state.typed.length>=q.answer.length||state.analysisWrong.includes(letter))return;
  ...
  else{state.analysisWrong.push(letter);state.analysisMisses++;setLetterFeedback(`${letter} ยังไม่ใช่ตัวถัดไป · ผิด ${state.analysisMisses}/5 ครั้ง`,'wrong')}
  $('#continue-btn').disabled=state.typed.length!==q.answer.length&&state.analysisMisses<5;$('#continue-btn').textContent=state.analysisMisses>=5?'ดูคำตอบ':'ตรวจคำตอบ';sfx(correct?'correct':'wrong');renderKeyboard();
}
...
document.addEventListener('keydown',e=>{const q=state.session[state.index];if(!q||q.mode!=='analysis'||!$('#screen-game').classList.contains('active')||state.checked)return;if(e.key==='1')useHint('first',5);else if(e.key==='2')useHint('vowel',10);else if(e.key==='3')useHint('translate',15);else if(/^[a-z]$/i.test(e.key))guessAnalysisLetter(e.key.toUpperCase());else if(e.key==='Backspace'&&state.typed.length&&state.analysisMisses<5){state.typed=state.typed.slice(0,-1);state.analysisWrong=[];state.analysisLastGuess=null;renderKeyboard();$('#continue-btn').disabled=true}});
